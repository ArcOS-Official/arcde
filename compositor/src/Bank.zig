...
/// Windows mid-teardown (`impl == .destroying`) still resolve by id, but
/// their title/app_id/pid accessors are `unreachable` by design — asking a
/// dying window for metadata panics and aborts the whole compositor. Every
/// IPC-bound read must go through here so a dying window is reported as gone
/// instead of killing the event loop.
fn liveWindowFromId(id: u64) ?*Window {
    const win = windowFromId(id) orelse return null;
    if (win.impl == .destroying) return null;
    return win;
}
...
fn makeCompositorWindow(win: *Window, alloc: Allocator) !protocols.Window {
    // Dying windows carry no title/app_id/pid (those accessors are
    // unreachable); the window_destroy event supersedes any snapshot that
    // would contain them. Callers translate this into "not found".
    if (win.impl == .destroying) return error.WindowGone;
...
// ---------------------------------------------------------------------------
// Output preview canvases — composited from window thumbnails.
//
// Why composite: no public wlroots API exposes output pixels to the
// compositor itself (that is what the screencopy protocol is for, and it
// is client-driven), but the 1fps window-thumb sweep above already yields
// small RGBA frames plus layout rects for every window. Painting those
// onto a dark output-sized canvas gives a recognizable "what's on that
// display" preview for the shell's window switcher, using only CPU memcpy
// math. Served as `output_image` for `capture_output` / `capture_full`.
// ---------------------------------------------------------------------------

const output_thumb_width: usize = 256;

var output_thumbs: std.AutoHashMap(u64, Thumb) = undefined;
var output_thumbs_init: bool = false;
// Guarded by thumbs_mu (shared with the window cache above).

pub const CanvasWin = struct {
    x: i32,
    y: i32,
    w: u32,
    h: u32, // layout-space rect
    tw: usize,
    th: usize, // thumb dims
    rgba: []const u8, // packed RGBA thumb, tw*th*4
};

/// Paint window thumbs onto a dark canvas of the output's aspect. Pure CPU
/// math on caller-owned memory: background fill + one nearest-neighbor
/// blit per intersecting window, all coordinates clamped.
pub fn composeOutputCanvas(
    alloc: Allocator,
    canvas_w: usize,
    canvas_h: usize,
    out_x: i32,
    ...
    out_w: usize,
    out_h: usize,
    wins: []const CanvasWin,
) ![]u8 {
    const out = try alloc.alloc(u8, canvas_w * canvas_h * 4);
    errdefer alloc.free(out);
    var i: usize = 0;
    while (i < canvas_w * canvas_h) : (i += 1) {
        out[i * 4 + 0] = 24;
        out[i * 4 + 1] = 24;
        out[i * 4 + 2] = 28;
        out[i * 4 + 3] = 255;
    }
    if (out_w == 0 or out_h == 0) return out;
    const ox0: i64 = out_x;
    const oy0: i64 = out_y;
    const ox1: i64 = ox0 + @as(i64, @intCast(out_w));
    const oy1: i64 = oy0 + @as(i64, @intCast(out_h));
    for (wins) |win| {
        if (win.w == 0 or win.h == 0 or win.tw == 0 or win.th == 0) continue;
        if (win.rgba.len < win.tw * win.th * 4) continue;
        const wx0: i64 = win.x;
        const wy0: i64 = win.y;
        const wx1: i64 = wx0 + @as(i64, @intCast(win.w));
        const wy1: i64 = wy0 + @as(i64, @intCast(win.h));
        const ix0 = @max(wx0, ox0);
        const iy0 = @max(wy0, oy0);
        const ix1 = @min(wx1, ox1);
        const iy1 = @min(wy1, oy1);
        if (ix1 <= ix0 or iy1 <= iy0) continue;
        const cx0: usize = @as(usize, @intCast(ix0 - ox0)) * canvas_w / out_w;
        const cx1: usize = @min(canvas_w, @as(usize, @intCast(ix1 - ox0)) * canvas_w / out_w);
        const cy0: usize = @as(usize, @intCast(iy0 - oy0)) * canvas_h / out_h;
        const cy1: usize = @min(canvas_h, @as(usize, @intCast(iy1 - oy0)) * canvas_h / out_h);
        if (cx1 <= cx0 or cy1 <= cy0) continue;
        var cy: usize = cy0;
        while (cy < cy1) : (cy += 1) {
            const ly: i64 = oy0 + @as(i64, @intCast(cy * out_h / canvas_h));
            // Canvas->layout truncates twice (there and at cx0/cy0), so
            // the mapped point can land a pixel above/left of the window
            // edge it was clamped to — floor the delta at zero (the far
            // side is clamped below). All operands are non-negative, so
            // @divTrunc is exact truncating division here.
            const dy: i64 = @max(0, ly - wy0);
            var sy: usize = @intCast(@divTrunc(dy * @as(i64, @intCast(win.th)), @as(i64, @intCast(win.h))));
            if (sy >= win.th) sy = win.th - 1;
            var cx: usize = cx0;
            while (cx < cx1) : (cx += 1) {
                const lx: i64 = ox0 + @as(i64, @intCast(cx * out_w / canvas_w));
                const dx: i64 = @max(0, lx - wx0);
                var sx: usize = @intCast(@divTrunc(dx * @as(i64, @intCast(win.tw)), @as(i64, @intCast(win.w))));
                if (sx >= win.tw) sx = win.tw - 1;
                @memcpy(out[(cy * canvas_w + cx) * 4 ..][0..4], win.rgba[(sy * win.tw + sx) * 4 ..][0..4]);
            }
        }
    }
    return out;
...
/// Refresh every live output's preview canvas from the window-thumb cache.
/// Main thread only (timer callback): window slices are stable here because
/// only this thread ever mutates `thumbs`.
fn sweepOutputThumbs() void {
    if (!output_thumbs_init or !thumbs_init) return;
    var oit = server.om.outputs.iterator(.forward);
    while (oit.next()) |out| {
        if (out.wlr_output == null) continue;
        const id = outputId(out);
        const box = out.current.box();
        const has_mode = out.current.mode != .none;
        const ow: usize = if (has_mode) blk: {
            const w, const _h = out.current.dimensions();
            _ = _h;
            break :blk w;
        } else 0;
        const oh: usize = if (has_mode) blk: {
            const _w, const h = out.current.dimensions();
            _ = _w;
            break :blk h;
        } else 0;
        if (ow == 0 or oh == 0) continue;
        const cw: usize = output_thumb_width;
        const ch: usize = @max(1, oh * cw / ow);
        var wins: std.ArrayList(CanvasWin) = .empty;
        defer wins.deinit(bank_alloc);
        var wit = server.wm.windows.iterator();
        while (wit.next()) |win| {
            if (win.impl == .destroying) continue;
            const wid = windowIdFromRef(win.ref);
            const th = thumbs.get(wid) orelse continue;
            if (th.rgba.len == 0) continue;
            wins.append(bank_alloc, .{
                .x = win.box.x,
                .y = win.box.y,
                .w = @intCast(@max(0, win.box.width)),
                .h = @intCast(@max(0, win.box.height)),
                .tw = @intCast(th.width),
                .th = @intCast(th.height),
                .rgba = th.rgba,
            }) catch continue;
        }
        const rgba = composeOutputCanvas(bank_alloc, cw, ch, box.x, box.y, ow, oh, wins.items) catch continue;
        thumbs_mu.lockUncancelable(ioForMainThread());
        if (output_thumbs.getPtr(id)) |old| {
            bank_alloc.free(old.rgba);
            old.* = .{ .width = @intCast(cw), .height = @intCast(ch), .rgba = rgba };
        } else {
            output_thumbs.put(id, .{ .width = @intCast(cw), .height = @intCast(ch), .rgba = rgba }) catch bank_alloc.free(rgba);
        }
        thumbs_mu.unlock(ioForMainThread());
    }
    // Prune canvases of dead outputs.
    var dead: std.ArrayList(u64) = .empty;
    defer dead.deinit(bank_alloc);
    var kit = output_thumbs.keyIterator();
    while (kit.next()) |k| {
        if (outputFromId(k.*) == null) dead.append(bank_alloc, k.*) catch {};
    }
    if (dead.items.len > 0) {
        thumbs_mu.lockUncancelable(ioForMainThread());
        defer thumbs_mu.unlock(ioForMainThread());
        for (dead.items) |id| {
            const kv = output_thumbs.fetchRemove(id);
            if (kv) |e| bank_alloc.free(e.value.rgba);
        }
    }
}

fn pruneOutputThumb(id: u64) void {
    if (!output_thumbs_init) return;
    thumbs_mu.lockUncancelable(ioForMainThread());
    defer thumbs_mu.unlock(ioForMainThread());
    const kv = output_thumbs.fetchRemove(id);
    if (kv) |e| bank_alloc.free(e.value.rgba);
...
// ---------------------------------------------------------------------------
// Thumbnail serving (capture_window / capture_output / capture_full).
// Per-frame pixel previews for the shell's window switcher, composited from
// live scene contents. Unlike client screen capture, these never leave the
// compositor process except as downscaled stills over the bank socket.
// ---------------------------------------------------------------------------
...
fn serveCaptureWindowMessage(alloc: Allocator, window_id: u64) !nilebank.Message {
    if (windowFromId(window_id) == null)
        return serveError(alloc, 2, "window not found");

    thumbs_mu.lockUncancelable(ioForRequestThread());
    const cached = thumbs.get(window_id);
    const rgba: []u8 = if (cached) |c| alloc.dupe(u8, c.rgba) catch &.{} else &.{};
    const cw: u32 = if (cached) |c| c.width else 0;
    const ch: u32 = if (cached) |c| c.height else 0;
    thumbs_mu.unlock(ioForRequestThread());
    if (rgba.len == 0)
        return serveError(alloc, 3, "no frame yet");
    return serveThumbMessage(alloc, .window, window_id, rgba, cw, ch);
}

/// Serve one `capture_output` from the composited preview canvas (see
/// sweepOutputThumbs). Same threading + shrink-to-fit contract as the
/// window path above.
fn serveCaptureOutputMessage(alloc: Allocator, output_id: u64) !nilebank.Message {
    if (outputFromId(output_id) == null)
        return serveError(alloc, 2, "output not found");

    thumbs_mu.lockUncancelable(ioForRequestThread());
    const cached = output_thumbs.get(output_id);
    const rgba: []u8 = if (cached) |c| alloc.dupe(u8, c.rgba) catch &.{} else &.{};
    const cw: u32 = if (cached) |c| c.width else 0;
    const ch: u32 = if (cached) |c| c.height else 0;
    thumbs_mu.unlock(ioForRequestThread());
    if (rgba.len == 0)
        return serveError(alloc, 3, "no frame yet");
    return serveThumbMessage(alloc, .output, output_id, rgba, cw, ch);
}

/// Serve `capture_full`: the primary (first listed) output's preview
/// canvas, or the named output when `output_id != 0`.
fn serveCaptureFullMessage(alloc: Allocator, output_id: u64) !nilebank.Message {
    if (output_id != 0) return serveCaptureOutputMessage(alloc, output_id);
    var it = server.om.outputs.iterator(.forward);
    while (it.next()) |out| {
        if (out.wlr_output == null) continue;
        return serveCaptureOutputMessage(alloc, outputId(out));
    }
    return serveError(alloc, 3, "no frame yet");
}

fn serveError(alloc: Allocator, code: u32, text: []const u8) !nilebank.Message {
    const ev: protocols.Event = .{ .error_msg = .{ .code = code, .message = try alloc.dupe(u8, text) } };
    defer ev.deinit(alloc);
    return try nilebank.encodeCompositorEvent(alloc, ev, protocols.encodingForEvent(.error_msg));
}

const ThumbTarget = enum { window, output };

/// Shrink owned RGBA until its deflated image event fits the frame budget,
/// then return the encoded message. `rgba` is freed exactly once on every
/// path; the event borrows it and is never passed to `deinit` (which would
/// free `rgba` out from under the retry loop).
fn serveThumbMessage(alloc: Allocator, target: ThumbTarget, id: u64, rgba: []u8, cw: u32, ch: u32) !nilebank.Message {
    var w: usize = cw;
    var h: usize = ch;
    var pixels: []u8 = rgba;
    while (true) {
        const ev: protocols.Event = switch (target) {
            .window => .{ .window_image = .{
                .window_id = id,
                .image = .{ .width = @intCast(w), .height = @intCast(h), .stride = @intCast(w * 4), .format = .rgba8, .data = pixels },
            } },
            .output => .{ .output_image = .{
                .output_id = id,
                .image = .{ .width = @intCast(w), .height = @intCast(h), .stride = @intCast(w * 4), .format = .rgba8, .data = pixels },
        ...
        const out = nilebank.encodeCompositorEvent(alloc, ev, protocols.encodingForEvent(tag)) catch {
            alloc.free(pixels);
            return serveError(alloc, 1, "out of memory");
        };
        ...
        const smaller = downscaleRgba(alloc, pixels, w, h, nw, nh) catch {
            alloc.free(pixels);
            return serveError(alloc, 1, "out of memory");
        };
        alloc.free(pixels);
...
fn collectWindows(alloc: Allocator) ![]protocols.Window {
    ...
    // MRU first; skip stale refs (destroyed since last prune) and
    // windows mid-teardown (no title/app_id/pid — see liveWindowFromId).
...
pub fn isRequestKeyboardFocusActive() bool {
    return request_keyboard_focus_active;
}
...
fn findLayerSurfaceByNamespace(namespace: []const u8) ?*LayerSurface {
    ...
    for ([_]zwlr.LayerShellV1.Layer{ .overlay, .top, .bottom, .background }) |layer| {
        ...
        while (it.next()) |node| {
            ...
            const wlr_layer_surface = layer_surface.wlr_layer_surface;
            if (!wlr_layer_surface.surface.mapped) continue;
            if (std.mem.eql(u8, std.mem.span(wlr_layer_surface.namespace), namespace)) return layer_surface;
        }
    }
    return null;
}
    ...
    for (ops) |op| {
        switch (op) {
            ...
            .request_keyboard_focus => |v| {
                ...
                const shell = target.?;
                const seat = server.input_manager.defaultSeat();
                const already_focused = seat.focused == .layer_surface and seat.focused.layer_surface == shell;
                if (already_focused) {
                    if (seat.shell_mod.focus_from_mod) {
                        ...
                        if (!request_keyboard_focus_active) {
                            request_keyboard_focus_prev = orig_prev;
                            request_keyboard_focus_shell = shell.ref;
                            request_keyboard_focus_active = true;
                ...
                if (!request_keyboard_focus_active) {
                    request_keyboard_focus_prev = switch (seat.focused) {
                    ...
                    // Update tracked shell to new target if different
                    request_keyboard_focus_shell = shell.ref;
                }
                seat.focus(.{ .layer_surface = shell });
            ...
            .release_keyboard_focus => {
                if (!request_keyboard_focus_active) {
                    log.info("bank: release_keyboard_focus: not active", .{});
                    broadcast(.{ .launcher_closed = {} });
                    return 0;
                }
                const seat = server.input_manager.defaultSeat();
                const shell_ref = request_keyboard_focus_shell;
                const on_shell = if (shell_ref) |r| seat.focused == .layer_surface and seat.focused.layer_surface.ref.key.index == r.key.index and seat.focused.layer_surface.ref.key.generation == r.key.generation else false;
                ...
                log.info("bank: release_keyboard_focus -> restored", .{});
                broadcast(.{ .launcher_closed = {} });
            },
            .refresh_shell_focus => {
                // Converge a (re)connected shell on the current focus state:
                // pushes only fire on edges, so without this a client that
                // connects while unfocused would wrongly assume focus.
                // Reset first to force the push even when the state matches
                // the last edge (which may have gone to zero listeners).
                const seat = server.input_manager.defaultSeat();
...
fn bankCallback(_: ?*anyopaque, msg: nilebank.Message) anyerror!nilebank.Message {
    ...
    if (req == .capture_output) {
        return serveCaptureOutputMessage(alloc, req.capture_output.output_id) catch |err| {
            log.warn("bank: output capture serve failed: {}", .{err});
            const ev: protocols.Event = .{ .error_msg = .{ .code = 1, .message = try alloc.dupe(u8, "capture failed") } };
            defer ev.deinit(alloc);
            return try nilebank.encodeCompositorEvent(alloc, ev, protocols.encodingForEvent(.error_msg));
        };
    ...
    if (req == .capture_full) {
        return serveCaptureFullMessage(alloc, req.capture_full.output_id) catch |err| {
            log.warn("bank: full capture serve failed: {}", .{err});
            const ev: protocols.Event = .{ .error_msg = .{ .code = 1, .message = try alloc.dupe(u8, "capture failed") } };
            defer ev.deinit(alloc);
            return try nilebank.encodeCompositorEvent(alloc, ev, protocols.encodingForEvent(.error_msg));
        };
    ...
    const ev: protocols.Event = switch (req) {
        ...
        .get_window => |v| blk: {
            // liveWindowFromId folds "mid-teardown" into "not found": a
            // dying window's metadata accessors would panic (see the
            // helper), and its destroy event is already on the way.
            if (liveWindowFromId(v.id)) |win| {
        ...
        // .capture_window/.capture_output/.capture_full are served above
        // from the thumbnail caches.
        .capture_window => unreachable,
        .capture_output => unreachable,
        .capture_full => unreachable,
        .focus_window => |v| blk: {
        ...
        .request_keyboard_focus => |v| blk: {
            const owned = alloc.dupe(u8, v.namespace) catch break :blk .{ .error_msg = .{ .code = 1, .message = try alloc.dupe(u8, "out of memory") } };
            queueAsync(.{ .request_keyboard_focus = .{ .namespace = owned } });
            break :blk .{ .pong = .{ .nonce = 0 } };
        },
...
// Last published shell keyboard-focus state (null = never published).
// Deduplicates the `shell_focus_changed` push so only real edges hit the
// socket: Seat.focus() calls publishShellFocus() on every focus change,
// and a refresh op re-publishes after (re)registration.
var last_shell_focus: ?bool = null;

/// Publish `shell_focus_changed` when `focused` differs from the last
/// published state. Main thread only (called from Seat.focus() and the
/// async-queue drain).
pub fn publishShellFocus(focused: bool) void {
    if (last_shell_focus) |last| {
        if (last == focused) return;
    }
    last_shell_focus = focused;
    log.info("bank: shell_focus_changed focused={}", .{focused});
    broadcast(.{ .shell_focus_changed = .{ .focused = focused } });
}
...
fn onCompositorEvent(event: Compositor.Event) void {
    switch (event) {
        .window_add => |win| {
            // A dying window carries no title; its destroy event (already
            // queued behind this one) tells the shell it is gone.
        ...
        .output_remove => |out| {
            const id = outputId(out);
            pruneOutputThumb(id);
            broadcast(.{ .output_removed = .{ .id = id } });
        },
        .output_update => |out| {
            const full = makeCompositorOutput(out, bank_alloc) catch return;
...
pub fn init() !void {
    ...
    thumbs_init = true;
    output_thumbs = std.AutoHashMap(u64, Thumb).init(bank_alloc);
    output_thumbs_init = true;
...
pub fn deinit() void {
    ...
    if (thumbs_init) {
        ...
        thumbs.deinit();
        var oit = output_thumbs.iterator();
        while (oit.next()) |kv| bank_alloc.free(kv.value_ptr.rgba);
        output_thumbs.deinit();
        thumbs_mu.unlock(ioForMainThread());
        thumbs_init = false;
        output_thumbs_init = false;
    }
