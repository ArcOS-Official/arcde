...
The shell renders live previews for the window switcher and output
canvases. These are per-frame stills the compositor composites for the
shell itself — not shares, and they never leave the process except as
downscaled images over the bank socket:

- `capture_window { window_id, scale }` → `window_image` (1fps per-window
  thumbnails).
- `capture_output { output_id, scale }` → `output_image` (per-display
  canvas composited from the window thumbs on the same 1fps sweep).
- `capture_full { output_id }` serves one display, `0` = primary.

Oversized payloads shrink until the deflated frame fits; error `3` ("no
frame yet") means back off and retry.
