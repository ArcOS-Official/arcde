...
// mic/camera/screen consumption (PipeWire node graph), and in-progress
// browser downloads (partial files in the download dir).
//
...
pub const Item = struct {
    ...
    detail: []const u8, // owned: "node 56" / ""
    stop_id: u64, // PipeWire node id; 0 = no stop
...
/// takeStops() on the worker: PipeWire node ids become `pw-cli destroy`
/// spawns.
...
fn pollPipeWire(self: *Activity) bool {
    ...
    return self.adoptPwDump(text);
}

/// Classify one pw-dump snapshot into stream entries and adopt on change.
/// Pure w.r.t. I/O (takes the dump text): unit-tested with canned JSON.
/// pw-dump repeats keys deep inside port params (duplicate "info"), so
/// duplicates keep the last value instead of failing the whole parse —
/// the default `.error` behavior silently disables every PipeWire
/// indicator (mic/camera/share).
fn adoptPwDump(self: *Activity, text: []const u8) bool {
    const parsed = std.json.parseFromSlice(std.json.Value, self.alloc, text, .{ .duplicate_field_behavior = .use_last }) catch return false;
    defer parsed.deinit();
...
test "activity: pw-dump with duplicate keys still detects mic" {
    // Real pw-dump repeats "info" deep inside port params; the parse must
    // tolerate that (use_last) instead of failing the whole snapshot.
    const t = std.testing;
    var act = Activity{};
    act.init(t.allocator, t.io);
    defer act.deinit();
    const dump =
        \\[
        \\  {"id": 58, "type": "PipeWire:Interface:Node", "info": {
        \\    "state": "running",
        \\    "props": {"media.class": "Audio/Source", "node.name": "alsa_input.usb-mic",
        \\              "application.name": "", "application.process.binary": "", "application.process.id": 0}}},
        \\  {"id": 106, "type": "PipeWire:Interface:Node", "info": {
        \\    "state": "running",
        \\    "props": {"media.class": "Stream/Input/Audio", "node.name": "OBS",
        \\              "application.name": "OBS", "application.process.binary": "obs",
        \\              "application.process.id": 1234}}},
        \\  {"id": 105, "type": "PipeWire:Interface:Link", "info": {
        \\    "state": "active", "output-node-id": 58, "input-node-id": 106}},
        \\  {"id": 33, "type": "PipeWire:Interface:Port", "info": {
        \\    "state": "suspended",
        \\    "params": {"Tag": [{"info": {}, "info": {}}]}}}
        \\]
    ;
    try t.expect(act.adoptPwDump(dump));
    try t.expectEqual(@as(usize, 1), act.streams.items.len);
    try t.expectEqual(Kind.mic, act.streams.items[0].kind);
    try t.expectEqual(@as(u64, 106), act.streams.items[0].node_id);
    try t.expectEqualStrings("OBS", act.streams.items[0].app);
    const c = act.counts();
    try t.expectEqual(@as(usize, 1), c.mic);
    // Second adopt with identical content reports no change.
    try t.expect(!act.adoptPwDump(dump));
}
