...
// UI-thread output-preview cache (see ImageEntry): composited display
...
pub fn initWithWakeup(
    ...
    self.output_images = ImageMap.init(alloc);
    self.req_q = .{};
    self.commit_q = .{};
    self.conn = null;
    self.conn_mu = .init;
...
pub fn deinit(self: *State) void {
    ...
    var oit = self.output_images.iterator();
    while (oit.next()) |kv| kv.value_ptr.deinit(self.alloc);
    self.output_images.deinit();
    self.output_images = ImageMap.init(self.alloc);
...
// UI thread only. Display-preview twin of windowImage(): returns the cached
// composited canvas for output `id` as a dvui.ImageSource, enqueueing an
// async capture_output on miss. Same TTL/coalescing/backoff contract.
pub fn outputImage(self: *State, id: u64) ?dvui.ImageSource {
    const now = self.nowMs();
    if (self.output_images.getPtr(id)) |e| {
        if (e.fetched_ms != 0 and now - e.fetched_ms < image_ttl_ms) {
            return e.imageSource();
        }
        if (self.captureBackedOff(now)) {
        ...
        e.requested_ms = now;
        self.req_q.push(self.alloc, self.io, .{ .capture_output = .{ .id = id, .scale = image_capture_scale } });
        if (e.fetched_ms == 0) return null;
        return e.imageSource();
    }
    if (self.captureBackedOff(now)) return null;
    self.output_images.put(id, .{ .requested_ms = now }) catch return null;
    self.req_q.push(self.alloc, self.io, .{ .capture_output = .{ .id = id, .scale = image_capture_scale } });
    return null;
}

// UI thread only. Display twin of prefetchWindowImages(): call when
// opening the share picker so output previews converge in one round trip.
pub fn prefetchOutputImages(self: *State) void {
    const now = self.nowMs();
    if (self.captureBackedOff(now)) return;
    for (self.outputs) |*o| {
        const e = self.output_images.getPtr(o.id);
        if (e) |entry| {
            if (entry.fetched_ms != 0 and now - entry.fetched_ms < image_ttl_ms) continue;
            if (now - entry.requested_ms < image_ttl_ms) continue;
            entry.requested_ms = now;
        } else {
            self.output_images.put(o.id, .{ .requested_ms = now }) catch continue;
        }
        self.req_q.push(self.alloc, self.io, .{ .capture_output = .{ .id = o.id, .scale = image_capture_scale } });
    }
}
...
pub fn worker(self: *State, io: std.Io) void {
    ...
    while (!self.stop.load(.seq_cst)) {
        ...
        // Drain control-center stop requests: PipeWire nodes are destroyed
        // in place; downloads have no in-shell cancel.
...
fn handleAction(self: *State, conn: *nilebank.Connection, a: *Action) !void {
    switch (a.*) {
        ...
        .capture_output => |c| {
            var ev = try conn.requestCompositor(
                .{ .capture_output = .{ .output_id = c.id, .scale = c.scale } },
                .raw,
            );
            var consumed = false;
            switch (ev) {
                .output_image => |*v| {
                    if (v.output_id == c.id) {
                        if (normalizeWindowImage(self.alloc, v.image)) |norm| {
                            ...
                            // Success clears any earlier backoff, like the
                            // window path above.
                            self.capture_backoff_until_ms.store(0, .seq_cst);
                            self.commitEvent(ev);
                            self.requestRefresh();
                ...
                .error_msg => |e| {
                    if (e.code == 3) {
                        const until = self.nowMs() + image_error_backoff_ms;
                        self.capture_backoff_until_ms.store(until, .seq_cst);
                        std.log.debug("output capture error {d}, backing off {d}ms", .{ e.code, image_error_backoff_ms });
                    }
                },
                else => {},
            }
...
fn findOutput(self: *State, id: u64) ?*proto.Output {
    for (self.outputs) |*o| if (o.id == id) return o;
    return null;
}

fn upsertOutput(self: *State, out: proto.Output) void {
    for (self.outputs) |*cur| {
        if (cur.id != out.id) continue;
        cur.deinit(self.alloc);
        cur.* = .{
            .id = out.id,
            .name = self.dupeNonEmpty(out.name),
            .make = self.dupeNonEmpty(out.make),
            .model = self.dupeNonEmpty(out.model),
            .x = out.x,
            .y = out.y,
            .mode = out.mode,
            .scale = out.scale,
    ...
    const old_len = self.outputs.len;
    self.outputs = self.alloc.realloc(self.outputs, old_len + 1) catch return;
    self.outputs[old_len] = .{
        .id = out.id,
        .name = self.dupeNonEmpty(out.name),
        .make = self.dupeNonEmpty(out.make),
        .model = self.dupeNonEmpty(out.model),
...
fn removeOutput(self: *State, id: u64) void {
    for (self.outputs, 0..) |*o, i| {
        if (o.id != id) continue;
        o.deinit(self.alloc);
        removeAt(proto.Output, self.alloc, &self.outputs, i);
        return;
    }
}
...
// Output twin of adoptImage(): adopts a worker-normalized output preview
// into the output_images map (same ownership steal).
fn adoptOutputImage(self: *State, id: u64, img: *proto.Image) void {
    const entry = self.output_images.getPtr(id) orelse {
        self.output_images.put(id, .{}) catch {
            if (img.data.len > 0) self.alloc.free(img.data);
            img.data = "";
            return;
        };
        const e = self.output_images.getPtr(id) orelse return;
        e.rgba = @constCast(img.data);
        e.width = img.width;
        e.height = img.height;
        e.fetched_ms = self.nowMs();
...
fn applyEvent(self: *State, ev: *proto.Event) void {
    switch (ev.*) {
        ...
        .output_image => |*v| self.adoptOutputImage(v.output_id, &v.image),
        .outputs_snapshot, .outputs => {
            for (self.outputs) |*o| o.deinit(self.alloc);
            if (self.outputs.len > 0) self.alloc.free(self.outputs);
            if (ev.* == .outputs_snapshot) {
                self.outputs = ev.outputs_snapshot.items;
                ev.outputs_snapshot.items = &.{};
            } else {
                self.outputs = ev.outputs.items;
                ev.outputs.items = &.{};
            }
        },
        .new_output, .output_added, .output_changed => |v| self.upsertOutput(v),
        .output_removed => |v| self.removeOutput(v.id),
